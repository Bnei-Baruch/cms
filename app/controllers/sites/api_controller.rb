class Sites::ApiController < ApplicationController
  ...
  def get_all_articles
    ...
    render :text => Proc.new { |response, output|
      ...
      output.write CSV.generate_line [:post_id, :post_date, :post_content, :post_title, :post_excerpt, :post_name, :post_thumbnail]
      output.write "\r\n"

      categories(3).each { |cat|
        articles(cat.id).each do |article|
          begin
            data = get_article_data(article)
            output.write((CSV.generate_line data.values_at(*column_names)).gsub!(/[\r\n]/, ' '))
            output.write "\r\n"
            i = i + 1
            GC.start if i % 500 == 0
          rescue
