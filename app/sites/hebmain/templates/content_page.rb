class Hebmain::Templates::ContentPage < WidgetManager::Template
  ...
  def ext_content
    WidgetManager::Base.new do
      w_class('cms_actions').new(:tree_node => @tree_node, :options => {:buttons => %W{ new_button edit_button delete_button }, :resource_types => %W{ article content_preview section_preview rss video},:new_text => 'צור יחידת תוכן חדשה', :has_url => false, :placeholder => 'main_content'}).render_to(self)
  ...
  def content_resources
    TreeNode.get_subtree(
      :parent => tree_node.id, 
      :resource_type_hrids => ['article', 'content_preview', 'section_preview', 'rss', 'video'], 
      :depth => 1,
      :has_url => false,
