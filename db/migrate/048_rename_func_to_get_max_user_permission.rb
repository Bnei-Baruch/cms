class RenameFuncToGetMaxUserPermission < ActiveRecord::Migration
  def self.up
    execute "DROP FUNCTION get_user_permeation(integer, integer)"
    execute "CREATE OR REPLACE FUNCTION get_max_user_permission(
  ...
  def self.down
    execute "DROP FUNCTION get_max_user_permission(integer, integer)"
