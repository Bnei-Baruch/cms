class CmsFunctionsV11 < ActiveRecord::Migration
  def self.up
    sql = <<-my_code
      ALTER TABLE tree_nodes ADD COLUMN max_user_permission integer;
      ALTER TABLE tree_nodes ADD CONSTRAINT max_user_permission_ck CHECK (max_user_permission is NULL);
      COMMENT ON COLUMN tree_nodes.max_user_permission IS 'This is fake field to overcome RoR constraints.';
      -- ################################################################################
      -- ################################################################################
      --
