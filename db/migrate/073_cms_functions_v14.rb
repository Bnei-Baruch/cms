class CmsFunctionsV14 < ActiveRecord::Migration
  def self.up
    sql = <<-my_code
      -- ################################################################################
      -- ################################################################################
      ...
      */
      ...
      DROP CAST IF EXISTS (boolean AS varchar);
      ...
      /**
       ...
       - p_where  		varchar 	: where clause for properties on hrids:
      					  hrid = value [and or] hrid ~ value, i.e. '(name ~ ''arvut'' or in_group) and (date > now() or date < now() + 15)'
