class TreeNodeCountChildren < ActiveRecord::Migration
  def self.up
        sql = <<-my_code
DROP FUNCTION IF EXISTS cms_treenode_direct_children(integer, integer, character varying[], boolean, boolean, character varying[], character varying[]);

CREATE OR REPLACE FUNCTION cms_treenode_direct_children(p_tn_id integer, p_user_id integer, p_res_hrids character varying[], p_is_main boolean, p_has_url boolean, p_placeholders character varying[], p_statuses character varying[], p_where character varying)
  RETURNS bigint AS
$BODY$
	...
	WHERE
	  ...
	  ($6 is null 			or a.placeholder = ANY ($6)) and
	  ($8 is null        		or a.resource_id IN (select * from cms_cache_resource_properties_where_pipe($8)))
...
$BODY$
		  ...
		  a.resource_status,
		  CASE
		    WHEN $12 = true THEN cms_treenode_direct_children(a.id, $2, $3, $4, $5, $10, $11, $6)
