class ResourceSitemap < ActiveRecord::Migration
  def self.up
    resource_type = ResourceType.find(:first, :conditions => ['hrid = ?', 'map'])
    if resource_type
      puts 'map already exists'
      return
    end

    puts 'Going to create notfound'

    resource_type = ResourceType.new(:name => 'Map',
      :name_code => '<title>',
      :hrid => 'map')
    raise 'Failed to create map' unless resource_type
    resource_type.save!
