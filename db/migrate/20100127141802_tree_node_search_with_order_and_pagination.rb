class TreeNodeSearchWithOrderAndPagination < ActiveRecord::Migration
  def self.up
    migration_login

    execute <<-SQL
CREATE OR REPLACE FUNCTION cms_treenode_subtree_order_asc(
	p_tn_id integer, 
	p_user_id integer, 
	p_res_hrids character varying[], 
	p_is_main boolean, 
	p_has_url boolean, 
	...
	p_statuses character varying[], 
	p_child_count boolean, 
	p_order_by character varying)
  RETURNS SETOF tree_nodes AS
$BODY$
	SELECT * FROM
		cms_treenode_subtree($1, $2, $3, $4, $5, $6, 0, 2147483647, $9, $10, $11, $12)
	      ORDER BY
		CASE
			WHEN $13 = 'id' THEN id
...
CREATE OR REPLACE FUNCTION cms_treenode_subtree_order_desc(
	p_tn_id integer, 
	p_user_id integer, 
	p_res_hrids character varying[], 
	p_is_main boolean, 
