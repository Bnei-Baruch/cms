...
DROP FUNCTION IF EXISTS cms_treenode_subtree(p_tn_id           integer, 
						...
						p_has_url      boolean,
						p_depth        integer,
						p_properties   cms_pair[] );
DROP FUNCTION IF EXISTS cms_pair_pipe(p_pairs cms_pair[]);
DROP FUNCTION IF EXISTS cms_properties_pipe(p_properties cms_pair[]);
...
CREATE OR REPLACE FUNCTION cms_pair_pipe(p_pairs cms_pair[]) RETURNS SETOF cms_pair AS $BODY$
DECLARE
  r integer;
  p cms_pair;
BEGIN
...
CREATE OR REPLACE FUNCTION cms_properties_pipe(p_properties cms_pair[]) RETURNS SETOF resource_properties AS $BODY$
DECLARE
...
/*
 ...
 - p_properties  cms_pair[]: array of property.hrid + resource_properties.* pair, i.e. ARRAY[ ROW('title','sdasf')::cms_pair, ROW('sub_title','')::cms_pair]
 ...
 Valid examples:
   ...
   select * from cms_treenode_subtree(17, cast(35 as integer), null, false, null, ARRAY[ ('title','sdasf')::cms_pair, ('sub_title','')::cms_pair])
...
CREATE OR REPLACE FUNCTION cms_treenode_subtree(p_tn_id        integer, 
						...
						p_has_url      boolean,
						p_depth        integer,
						p_properties   cms_pair[] ) RETURNS SETOF tree_nodes AS $BODY$
DECLARE
