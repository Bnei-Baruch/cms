// Keep track of already created dropzones and buttons in order not to create the same ones twice (HEB-445)
var dz_list = [];
var buttons_list = [];
...
function tree_drop_zone(widget_node_id, url, widget, updatable, updatable_view_mode) {
    if (jQuery.inArray(widget_node_id, dz_list) != -1) {
        return;
    }
    dz_list.push(widget_node_id);
