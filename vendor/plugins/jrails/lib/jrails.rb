    ...
    module ScriptaculousHelper
      ...
      def visual_effect(name, element_id = false, js_options = {})
        ...
        if ['fadeIn','fadeOut','fadeToggle'].include?(name)
          ...
          javascript << ");"
        elsif name.eql?("animate")
          javascript = "#{JQUERY_VAR}('#{jquery_id(element_id)}').animate("
          javascript << ",#{options_for_javascript(js_options)}" unless speed.nil? && js_options.empty?
          javascript << ",#{speed}" unless speed.nil?
          javascript << ");"
        elsif name.eql?('highlight') || (name == :highlight)
          start_color = "#ffff99"
          end_color = "#ffffff"
          start_color = js_options[:start_color] if js_options.has_key? :start_color
          end_color = js_options[:end_color] if js_options.has_key? :end_color

          javascript = "jQuery('#{jquery_id(element_id)}').animate({backgroundColor:'#{start_color}'}, 500)"
